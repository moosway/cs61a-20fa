...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(self,ContainerAnt) and ContainerAnt.can_contain(self, place.ant):
                self.contain_ant(place.ant)
            elif isinstance(place.ant,ContainerAnt) and ContainerAnt.can_contain(place.ant,self) and place.ant.contained_ant==None:
    ...
    def nearest_bee(self, beehive):
        ...
        while i<=self.max_range:
            if near==beehive:
                return None
            if near.bees!=[]:
                return rANTdom_else_none(near.bees)
            near=near.entrance
            i+=1
        return None
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    ''' the WallAnt, which is an ant that does nothing each turn. '''
    name='Wall'
    food_cost = 4
    implemented = True   # Change to True to view in the GUI
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.can_queen:
            self.iftrue=True
            QueenAnt.can_queen=False
        else:
            self.iftrue=False
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.iftrue:
            ...
            while place.exit!=None:
                place=place.exit
                if place.ant!=None and place.ant.double:
    ...
    def reduce_armor(self, amount):
        ...
        if self.iftrue and self.armor<=0:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        """
        if self.move!=[]:
            self.move[0](self.pure_action,self)(gamestate)
            self.move=self.move[1:]
        else:
            self.pure_action(gamestate)
            

    def pure_action(self, gamestate):
...
class BodyguardAnt(ContainerAnt):
    ...
    # END Optional 2
    def __init__(self, *args, **kwargs):
        ContainerAnt.__init__(self, *args, **kwargs,armor=2)

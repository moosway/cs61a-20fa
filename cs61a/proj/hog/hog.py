...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who == 0:
            ...
            score1=score1+take_turn(strategy1(score1,score0),score1,dice)
            say=say(score0, score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def high(score0,score1):
        if who==0:
            get=score0-last_score
            if get>running_high and get>0:
                print(get,'point(s)! The most yet for Player 0')
                return announce_highest(who,score0,get)
            return announce_highest(who,score0,running_high)
        else:
            get=score1-last_score
            if get>running_high and get>0:
                print(get,'point(s)! The most yet for Player 1')
                return announce_highest(who,score1,get)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    for i in range(1,11):
        old=make_averaged(roll_dice,trials_count)
        old_d=old(i,dice)
        if old_d>new_d:
            new_d=old_d
            t=i
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    a=free_bacon(opponent_score)
    if extra_turn(score+a,opponent_score):
        return 0

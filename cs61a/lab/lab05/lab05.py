...
def berry_finder(t):
    ...
    if is_leaf(t):
        if label(t)=='berry':
            return True
        return False
    bol=berry_finder(label(t))
    for branch in branches(t):
        bol= (bol or berry_finder(branch))
    return bol
...
def riffle(deck):
    ...
    for i in range(len(deck)//2):
        deck_n=deck_n+[deck[i]]+[deck[i+len(deck)//2]]
    return deck_n
...
def add_trees(t1, t2):
    ...
    return tree(label(t1)+label(t2),[add_trees(branch1,branch2) for branch1,branch2 in zip_longest(branches(t1),branches(t2))])

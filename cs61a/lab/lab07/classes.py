...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for _ in range(3):
            opponent.hand=opponent.hand[1:]+[opponent.deck.draw()]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        while j<len(opponent.deck.cards):
            if opponent.deck.cards[j].attack==other_card.attack and opponent.deck.cards[j].defense==other_card.defense:

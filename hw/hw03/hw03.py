...
def balanced(m):
    ...
    if is_mobile(end(left(m))):
        ba=ba and balanced(end(left(m)))
    if is_mobile(end(right(m))):
        ba=ba and balanced(end(right(m)))
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        return tree(la)
    return tree(la,[replace_leaf(branch,find_value,replace_value) for branch in branches(t)])
...
def has_path(t, word):
    ...
    if label(t)==word[0]:
        for branch in branches(t):
            ta=ta or has_path(branch,word[1:])
    return ta
...
def multiple_references_explanation():
    return """The multiple reference problem will expand the exact interval, for example
    the first reference use the lower_bound, and the second reference use the upper_bound, 
    although the values are equal in math, the interval will expand
    """
